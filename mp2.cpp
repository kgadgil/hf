...
vector<vector<double>> hf(BasisSet obs, MatrixXd overlap, MatrixXd Core, int itr, vector<Atom> atoms, int occ, MatrixXd &evec) {
//initial guess
...
int noddy (BasisSet obs, MatrixXd evec) {
	vector<double> intg;
	intg = tensor(obs);
	vector<double> pqrs;
//	printVec(intg);
		...
		for (int q = 0; q < obs.nbf(); q++) {
			for (int r = 0; r < obs.nbf(); r++) { 
				for (int s = 0; s < obs.nbf(); s++) {
					double sum = 0;
					for (int u = 0; u < obs.nbf(); u++) {
						for (int v = 0; v < obs.nbf(); v++) { 
							for (int l = 0; l < obs.nbf(); l++) {
								for (int i = 0; i < obs.nbf(); i++) {
									sum += evec(u,p) * evec(v,q) * evec(r,l) * evec(s,i) * intg[u+v+r+s];
								}
							}
						}
					}
...
int mp2 (BasisSet obs, MatrixXd evec) {
	vector<double> intg;
	intg = tensor(obs);
	vector<double> pqrs;
//	printVec(intg);
...
//print basis set
	...
	cout << "HF" << endl;
	MatrixXd evec;
	hf(obs, S, coreH, 10, atoms, occ, evec);
	cout << "Eigenvectors \n" << evec << endl;
